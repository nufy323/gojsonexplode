...
func TestEnableExplodeNestedArray(t *testing.T) {
	input := `
    [
        {
...
func TestDisableExplodeNestedArray(t *testing.T) {
	input := `
    [
        {
            "description": "a schema given for items",
	...
	output := `{"0.description":"a schema given for items","0.schema.items.type":"integer","0.tests":[{"data":[1,2,3],"description":"valid items","valid":true},{"data":[1,"x"],"description":"wrong type of items","valid":false},{"data":{"foo":"bar"},"description":"ignores non-arrays","valid":true}],"1.description":"an array of schemas for items","1.schema.items":[{"type":"integer"},{"type":"string"}],"1.tests":[{"data":[1,"foo"],"description":"correct types","valid":true},{"data":["foo",1],"description":"wrong types","valid":false}]}`
	ExplodeNestedList(false)
	out, _ := Explodejsonstr(input, ".")
	if out != output {
		t.Error("got", out)
	}
